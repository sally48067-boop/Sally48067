...
5. 页码体系必须先统一：默认使用 Book 印刷页码，不使用 PDF 文件页序。若材料混用了 PDF 页序和 Book 印刷页码，请先列为“页码体系需统一说明”，不要直接把可解释的页码差异判为硬错。
6. 无文字图片页可以用于 Picture Walk。若 Book 印刷 P2 是无文字场景图，TG 使用 P2 观察场景本身可以成立；真正需要反馈的是后续正文页、图片内容、问题或讲解发生错位。
...
7. Phonics 不只检查格式一致，还要判断规则名称是否科学、是否适合该级别教学。若规则本身不自然或不标准，例如把 mall 标成 open aw，应建议统一为更合理的表达，如 all word family /ɔːl/ 或 a in mall /ɔː/；若必须沿用大纲原写法，至少要求 Book/RR/TG 三处完全一致。
...
6. Worksheet 每一道封闭题都必须独立作答一次：逐题检查题干、图片、词库、选项、答案线和 TG Answer Key 是否能推出唯一答案。若词库没有对应答案、图片无法对应句子、或 TG 答案与学生页不一致，必须列为正式问题。
7. TG 中 Worksheet 页码、题型名称、活动说明、Answer Key 必须和学生 Worksheet 页面逐项对齐。若 TG 写 Worksheet 5 是 Drawing，但学生页 Worksheet p.5 实际是 T/F、p.6 才是 Drawing，应列为硬错或需确认。
...
- 匹配题或补全题必须检查图片是否能唯一支持目标句。如果图片只表现“大商场/室内场景”，不能清楚支持 I walk around with my family，应列为“需确认/可优化”；如果题目词库没有图片对应答案，应列为硬错。
...
3. 未上传材料不得凭空审核。
4. 不要把视觉抽取造成的空格、断词、横线数量误读直接列为正式问题。只有渲染页面视觉上确实存在问题，才可列入 issues。
5. 同一根因只保留一条主问题，相关位置写入 problem 或 action，不要重复拆成多条。例如 Worksheet 乱序词错误与 TG Answer Key 同步错误，应合并为同一根因问题。
6. 严重程度必须稳定：
   - 硬错：答案无法成立、词汇身份混淆、TG/Worksheet页码或答案 key 错位、正文事实冲突。
   - 需确认：依赖项目命名规则、视觉图片是否足够清晰、页码体系需统一。
   - 可优化：False 句自然度、低龄表达更自然、版面或图片可更贴合但不影响作答。
